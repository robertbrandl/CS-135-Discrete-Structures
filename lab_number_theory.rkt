...
;; 1 PTS
(define (divides? a b) (if (and(= a 0) (= b 0)) #t (if (= a 0) #f(if(= 0 (modulo b a)) #t #f))))
...
;; 1 PTS
(define (lcm a b) (if (and(= a 0) (= b 0)) 0 (/(* a b) (gcd a b))))
...
;; 2 PTS
(define (wj-solvable? cA cB g) (if (or(> cA (- g 1)) (> cB (- g 1))) (if (=(modulo g (gcd cA cB)) 0) #t #f) #f))
...
;; 4 PTS
(define (waterjug cA cB g) (waterHelper cA cB g 0 0 (list(list 0 0))))
(define (waterHelper cA cB g fA fB steps) (cond   ((= fB g) steps)
                                                  ((= fB cB) (waterHelper cA cB g fA 0 (append steps (list(list fA 0)))))
                                                  ((= fA 0) (waterHelper cA cB g cA fB (append steps (list(list cA fB)))))
                                                  ((> (+ fA fB) cB) (waterHelper cA cB g (- fA (- cB fB)) cB (append steps (list(list(- fA (- cB fB)) cB)))))
                                                  (else (waterHelper cA cB g 0 (+ fA fB) (append steps (list(list 0 (+ fA fB))))))) )
...
;; 5 PTS
(define (factorize n) (if (= n 0) (list 0) (if (= n 1) (list 1) (helper n 2))))

(define (helper n p) (if (= n 1) (list) (if (=(modulo n p) 0) (cons p (helper (quotient n p) p)) (helper n (+ p 1)))))
...
;; 2 PTS
(define (prime? n)(if (and(> n 1) (eq? n (car(factorize n)))) #t #f))
...
;; 3 PTS
(define (cong-mod? m) (lambda (a b) (if (= (modulo a m) (modulo b m)) #t #f)))
