...
;; 3 PTS
(define (get-vertices G) (usort(vHelper G)))
(define (vHelper G) (if (null? G) (list) (append (list(caar G)) (list(cadar G)) (vHelper (cdr G)))))
...
(define (degree G v)
  (if (null? G)
      0
      (if (dHelper (car G) v)
          (if (equal? (caar G) (cadr (car G)))
          (+ 2 (degree (cdr G) v))
          (+ 1 (degree (cdr G) v)))
          ( + 0 (degree (cdr G) v)))))

(define (dHelper e v) (if (or (equal? (car e) v) (equal? (cadr e) v)) #t #f))
...
;; 3 PTS
(define (all-even? G) (if (null?(remove(map even? (map (lambda x (degree G (car x))) (get-vertices G))) (list #t))) #t #f))
...
;; 4 PTS
(define (adjacent G v) (usort(ahelp G v)))

(define (ahelp G V) (if (null? G) (list) (if (equal? (caar G) V) (append (list (car(cdr(car G)))) (ahelp (cdr G) V))
                                             (if (equal? (car(cdr(car G))) V) (append (list (car(car G))) (ahelp (cdr G) V))  (append (ahelp (cdr G) V))))))
...
;; 4 PTS
(define (connected? G) (if (null? G) #t (if (null?(remove (get-vertices G) (usort(dfs G (caar G))) )) #t #f)))
...
;; 2 PTS
(define (eulerian? G) (if (and (connected? G) (all-even? G)) #t #f))