...
;; 5 PTS
(define (compose S R) (difference (make-set(if (null? R) '() (append(findSet(relate (car(cdr (car R))) S) (caar R)) (compose S (cdr R))))) (list(list))))
(define (findSet R x) (if (null? R) '() (cons(list x (car R)) (findSet (cdr R) x))))
...
;; 5 PTS
(define (power k R) (if (= k 0) '() (if (= k 1) R (compose R (power (- k 1) R)))))
...
;; 5 PTS
(define (transitive-closure R)  (closurehelp (cardinality R) R))
(define (closurehelp x R) (if (= x 0) '() (union(power x R) (closurehelp (- x 1) R))) )
...
;; 3 PTS
(define (transitive? R) (if (subset? (transitive-closure R) R) #t #f))
...
;; 2 PTS
(define (equivalence? n R)(if (and(eq? (symmetric? R) #t) (eq? (transitive? R) #t) (eq? (reflexive? n R) #t)) #t #f))
...
;; 3 Extra Credit PTS
(define (involutive? n R)(if (and (eq?(symmetric? R)#t) (eq?(injective? R)#t) (subset? (make-list n) (firstp(converse R)) )) #t #f))
(define (make-list n) (if (= n 1) (list 1) (append (list n) (make-list (- n 1)))))
(define (firstp R) (if (null? R) '() (append (list(caar R)) (firstp (cdr R)))))
