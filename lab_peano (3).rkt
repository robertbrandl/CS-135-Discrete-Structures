...
;; 1 PTS
(define (pred p)(if (pz? p) (error-predecessor) (car p)))
...
;; 2 PTS
(define (p+ a b)(if (pz? a) b (p+ (pred a) (succ b))))
...
;; Type signature: (p- peano peano) -> peano
;; 2 PTS
(define (p- a b) (if (pz? b) a (if (pz? a) (error-subtraction) (p- (pred a) (pred b)))))
...
;; Type signature: (p* peano peano) -> peano
;; 2 PTS
(define (p* a b) (if (pz? b)  z (if (pz? a) z (mhelper a b z))))
(define (mhelper a b multi) (if (pz? b)  multi (mhelper a (pred b) (p+ multi a))))
...
;; Type signature: (p^ peano peano) -> peano
;; 1 PTS
(define (p^ a b) (if (pz? b)  (succ z) (if (pz? a) z (ehelper a b a))))
(define (ehelper a b multi) (if (pz? (pred b))  multi (ehelper a (pred b) (p* multi a))))
...
;; 2 PTS
(define (p= a b) (if (or (pz? b) (pz? a)) (if (and (pz? b)(pz? a)) #t #f) (p= (pred a) (pred b))))
...
;; Type signature: (p> peano peano) -> boolean
;; 2 PTS
(define (p> a b)(if (or (pz? b) (pz? a)) (if (and(pz? b) (not (pz? a))) #t #f) (p> (pred a) (pred b))))
...
;; Type signature: (p% peano peano) -> peano
;; 2 PTS
(define (p% a b)(if (pz? b) (error-modulo) (if (p> b a) a (shelper a b a))))
(define (shelper a b multi) (if (p> b multi)  multi (shelper a  b (p- multi b))))
...
;; 1 PTS
(define (p-divides? a b) (if (pz? a) (if (pz? b) #t #f) (if (pz? b) #t(if (pz?(p% b a)) #t #f))))
...
;; 3 PTS
(define (p-prime? p)(if (pz? p) #f (primehelp p (pred p))))
(define (primehelp p div) (if (p= div (succ z)) #t (if (p-divides? div p) #f (primehelp p (pred div))) ))