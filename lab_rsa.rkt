...
;; 3 PTS
(define (generate p q e) (list (list (* p q) e) (list (* p q) (mul-inv e (lcm (- p 1) (- q 1))))))
...
;; 5 points
(define (mod-exp b e m)(if (= e 0) 1 (if (= (modulo e 2) 0) (modulo (expt (mod-exp b (/ e 2) m) 2) m ) (modulo (* (mod-exp b (- e 1) m) b) m))))
...
;; 6 PTS
(define (encrypt plaintext key) (map (lambda x (mod-exp (caar x)  (car(cdr key)) (car key))) (convert plaintext)) )
(define (convert p) (map list (map char->integer (string->list p))))
...
;; 6 PTS
(define (decrypt ciphertext key) (list->string (map integer->char (map (lambda x (mod-exp (car x)  (car(cdr key)) (car key))) ciphertext))))
