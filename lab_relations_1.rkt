...
;; 3 PTS
(define (id n) (if (= n 0) '() (cons (list n n) (id (- n 1)))))
...
;; 2 PTS
(define (reflexive? n R) (if (subset? (id n) R) #t #f))
...
;; 2 PTS
(define (reflexive-closure n R) (union R (id n)))
...
;; 3 PTS
(define (converse R) (map reverse R))
...
;; 2 PTS
(define (symmetric? R)(if (subset? (converse R) R) #t #f))
...
;; 2 PTS
(define (symmetric-closure R)(union R (converse R)))
...
;; 3 PTS
(define (relate x R) (if (null? R) '() (if (eq?(caar R) x) (cons(car(cdr (car R))) (relate x (cdr R))) (relate x (cdr R)))))
...
;; 3 PTS
(define (injective? R) (if (eq?(cardinality(rel R)) (length (rel R))) #t #f))
(define (rel R) (if (null? R) '() (cons(car(cdr (car R))) (rel (cdr R)))))
